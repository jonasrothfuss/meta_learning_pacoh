...
class GPRegressionLearnedVI:
    ...
    def __init__(self, train_x, train_t, lr_params=1e-3, num_iter_fit=10000, likelihood_factor=1.0, feature_dim=2,
        ...
        self.num_iter_fit, self.likelihood_factor, self.feature_dim = num_iter_fit, likelihood_factor, feature_dim
        ...
        self.svi = SVI(self.model, self.guide, self.optimizer, num_samples=20, loss=Trace_ELBO())
    ...
    def fit(self, verbose=True, valid_x=None, valid_t=None, log_period=1000):
        ...
        for param_name, param_value in pyro.get_param_store().named_parameters():
            try:
                print("{:<30}{:<30}".format(param_name, param_value.item()))
            except:
                pass
    ...
    def predict(self, test_x, n_posterior_samples=100, return_density=False):
        """
        ...
        with torch.no_grad():
            ...
            pred_dist_mixture = EqualWeightedMixtureDist(pred_dists)

            if return_density:
                return pred_dist_mixture
            else:
                pred_mean = pred_dist_mixture.mean.numpy()
                pred_std = pred_dist_mixture.stddev.numpy()
    ...
    def _setup_model_guide(self, mean_module_str, covar_module_str, mean_nn_layers, kernel_nn_layers):
        ...
        def model(x_tensor, t_tensor):
            ...
            exponent_fn = lambda t_tensor: self.likelihood_factor * mll_fn(pred, t_tensor)
        ...
        def guide(x_tensor, t_tensor):
            # mean function posterior

            if mean_module_str == 'NN':
                lifted_mean_nn = _lift_nn_posterior(mean_nn, "mean_nn")
                nn_mean_fn = lifted_mean_nn()
                mean_module = None
            elif covar_module_str == 'constant':
                constant_mean_q_loc = pyro.param("constant_mean_q_loc", 0.0 * _ones)
                constant_mean_q_scale = pyro.param("constant_mean_q_scale", 1.0 * _ones)
                constant_mean = pyro.sample("constant_mean", Normal(constant_mean_q_loc, constant_mean_q_scale))
                mean_module = ConstantMeanLight(constant_mean)
                nn_mean_fn = None
            else:
                mean_module = gpytorch.means.ZeroMean()
            ...
            if covar_module_str == 'NN':
                lifted_kernel_nn = _lift_nn_posterior(kernel_nn, "kernel_nn")
                nn_kernel_fn = lifted_kernel_nn()
                kernel_dim = self.feature_dim
            else:
                nn_kernel_fn = None
            ...
            lengthscale_q_loc = pyro.param("lengthscale_q_loc", 0.0 * torch.ones(kernel_dim))
            lengthscale_q_scale = pyro.param("lengthscale_q_scale", 1.0 * torch.ones(kernel_dim))
            lengthscale = pyro.sample("lengthscale", LogNormal(lengthscale_q_loc, lengthscale_q_scale).to_event(1))

            outputscale_q_loc = pyro.param("outputscale_q_loc", torch.tensor(0.0))
            outputscale_q_scale = pyro.param("outputscale_q_scale", torch.tensor(1.0))
            outputscale = pyro.sample("outputscale", LogNormal(outputscale_q_loc, outputscale_q_scale))

            covar_module = SEKernelLight(lengthscale, outputscale)

            # noise variance posterior
            noise_var_q_loc = pyro.param("noise_var_q_loc", torch.tensor([0.0]))
            noise_var_q_scale = pyro.param("noise_var_q_scale", torch.tensor([1.0]))
            noise_prior = {'noise_covar.raw_noise': Normal(noise_var_q_loc, noise_var_q_scale)}
            lifted_likelihood = pyro.random_module('likelihood', gpytorch.likelihoods.GaussianLikelihood(), noise_prior)
            likelihood = lifted_likelihood()
...
def _lift_nn_posterior(net, name):
    vi_posteriors = {}
    for name, param in net.named_parameters():
        ...
        if 'bias' in name:
            vi_posteriors[name] = Normal(loc=vi_mu_param, scale=vi_sigma_param).to_event(1)
        if 'weight' in name:
            vi_posteriors[name] = Normal(loc=vi_mu_param, scale=vi_sigma_param).to_event(2)
...
if __name__ == "__main__":
    ...
    x_train = np.random.uniform(-1, 1, size=(40, 1))
    t_train = fun(x_train)

    x_test = np.random.uniform(-1, 1, size=(100, 1))
    t_test = fun(x_test)

    model = GPRegressionLearnedVI(x_train, t_train, )
    model.fit(valid_x=x_train, valid_t=t_train)