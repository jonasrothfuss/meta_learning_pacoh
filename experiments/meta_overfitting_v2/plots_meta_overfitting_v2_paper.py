...
for metric in metrics:
    ...
    lines.append(axes[0].plot(y_mean, label=str(metric))[0])
    axes[0].fill_between(x, y_mean - y_std * (1.96/np.sqrt(25)), y_mean + y_std*(1.96/np.sqrt(25)), alpha=0.2)
    axes[0].set_title('Sinusoid (n_context_samples = %i)'%n_context_samples)
...
for metric in metrics:
    ...
    lines.append(axes[1].plot(y_mean, label=str(metric))[0])
    axes[1].fill_between(x, y_mean - y_std * (1.96/np.sqrt(25)), y_mean + y_std * (1.96/np.sqrt(25)), alpha=0.2)
    axes[1].set_title('Cauchy (n_context_samples = %i)'%n_context_samples)
...
axes[1].legend(axes[0].lines, ('meta-train tasks','meta-test tasks') )
fig.suptitle(ALGO)
fig.tight_layout(rect=[0, 0, 1, 0.95])
fig.show()
#fig.savefig('meta_overfitting_v2_%s_%i.pdf'%(ALGO, n_context_samples))
